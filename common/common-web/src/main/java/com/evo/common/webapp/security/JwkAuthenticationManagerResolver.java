...
public class JwkAuthenticationManagerResolver implements AuthenticationManagerResolver<HttpServletRequest> {
    ...
    private final Map<String, AuthenticationManager> authenticationManagers =
            new HashMap<>(); // Cache các authentication manager
    ...
    public AuthenticationManager resolve(HttpServletRequest request) {
        return this.authenticationManagers.computeIfAbsent(
                toIssuerId(request), this::fromIssuer); // Lấy hoặc tạo authentication manager từ issuer
    }
    ...
    private String toIssuerId(HttpServletRequest request) { // Xác định issuer từ request
        ...
        try {
            if (StringUtils.hasText(
                    (String) JWTParser.parse(token).getJWTClaimsSet().getClaim("userId"))) {
                return "internal";
